...
public class SortCompare { 
    ...
    public static double time(String alg, Double[] a) { 
        ...
        else if (alg.equals("MergeS"))          MergeS.sort(a); 
        else if (alg.equals("MergeXS"))         MergeXS.sort(a); 
    ...
    // Use alg to sort trials sorted random arrays of length n. 
    ...
    // Use alg to sort trials nearly sorted random arrays of length n. 
    public static double timeNSInput(String alg, int n, double x, int trials) {
        double total = 0.0;
        Double[] a = new Double[n];
        // Perform one experiment (generate and sort an array).
        for (int t = 0; t < trials; t++) {
            for (int i = 0; i < n; i++)
                a[i] = i + StdRandom.gaussian(0, x);
            total += time(alg, a);
        } 
        return total; 
    } 
    ...
    public static void main(String[] args) { 
	    ...
	    StdOut.printf("[%.2f vs %.2f]\n", time1, time2); 
        } else if (args.length >= 5 && args[4].equals("nearlySorted")) {
	    double x;
	    if (args.length > 5)
		x = Double.parseDouble(args[5]);
	    else
		x = 10.0;
            time1 = timeNSInput(alg1, n, x, trials);   // Total for alg1. 
            time2 = timeNSInput(alg2, n, x, trials);   // Total for alg2.
	    StdOut.printf("For %d nearly sorted Doubles\n    %s is", n, alg1); 
	    StdOut.printf(" %.1f times faster than %s\n", time2/time1, alg2); 
	    StdOut.printf("[%.2f vs %.2f]\n", time1, time2); 
        } else {
